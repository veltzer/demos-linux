...
typedef struct _mypthread_rwlock_t {
	pthread_cond_t mycond_readers;
	pthread_cond_t mycond_writers;
...
int mypthread_rwlock_init(mypthread_rwlock_t* lock) {
	CHECK_ZERO(pthread_cond_init(&lock->mycond_readers, NULL));
	CHECK_ZERO(pthread_cond_init(&lock->mycond_writers, NULL));
...
int mypthread_rwlock_destroy(mypthread_rwlock_t* lock) {
	CHECK_ZERO(pthread_cond_destroy(&lock->mycond_readers));
	CHECK_ZERO(pthread_cond_destroy(&lock->mycond_writers));
...
int mypthread_rwlock_rdlock(mypthread_rwlock_t* lock) {
	...
	while(lock->writers>0) {
		CHECK_ZERO(pthread_cond_wait(&lock->mycond_readers, &lock->mymutex));
	}
	lock->readers++;
	lock->readers_waiting--;
...
int mypthread_rwlock_wrlock(mypthread_rwlock_t* lock) {
	...
	while(lock->readers>0 || lock->writers>0) {
		CHECK_ZERO(pthread_cond_wait(&lock->mycond_writers, &lock->mymutex));
	}
	lock->writers++;
	lock->writers_waiting--;
	CHECK_ZERO(pthread_mutex_unlock(&lock->mymutex));
...
int mypthread_rwlock_unlock(mypthread_rwlock_t* lock) {
	...
	if(lock->readers) {
		...
		if(lock->writers_waiting>0 && lock->readers==0) {
			...
			CHECK_ZERO(pthread_cond_signal(&lock->mycond_writers));
		} else {
			// the lock->writers==0 is redundant in the next line
			if(lock->readers_waiting>0 && lock->writers==0) {
				CHECK_ZERO(pthread_cond_broadcast(&lock->mycond_readers));
			}
		}
	} else {
		// I am a writer
