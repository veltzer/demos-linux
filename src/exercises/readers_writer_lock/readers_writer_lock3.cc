...
int mypthread_rwlock_init(mypthread_rwlock_t* lock) {
	CHECK_ZERO_ERRNO(pthread_cond_init(&lock->mycond_readers, NULL));
	CHECK_ZERO_ERRNO(pthread_cond_init(&lock->mycond_writers, NULL));
...
int mypthread_rwlock_destroy(mypthread_rwlock_t* lock) {
	CHECK_ZERO_ERRNO(pthread_cond_destroy(&lock->mycond_readers));
	CHECK_ZERO_ERRNO(pthread_cond_destroy(&lock->mycond_writers));
...
int mypthread_rwlock_unlock(mypthread_rwlock_t* lock) {
	...
	if(lock->readers) {
		...
		if(lock->writers_waiting>0 && lock->readers==0) {
			...
			CHECK_ZERO_ERRNO(pthread_cond_signal(&lock->mycond_writers));
		} else {
			// the lock->writers==0 is redundant in the next line
			if(lock->readers_waiting>0 && lock->writers==0) {
				CHECK_ZERO_ERRNO(pthread_cond_broadcast(&lock->mycond_readers));
			}
		}
	} else {
		// I am a writer
