 ...
 * do not allow ptrace (which is at the core of gcore) of any process to any
 * process
 * (ubuntu is a prime example). The default policy (which can be configured by
 * /proc)
 * is that only the parent of a process can photo the child.
 *
 * Actually, Im using sudo here so Im not really using ptrace right. This is
 * because
 * the process that Im taking a photo of and the gcore process are siblings and
 * not
 * parent-child. There is probably a way to make this work if we link with
 * libgdb and call
